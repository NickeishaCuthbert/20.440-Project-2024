...
setwd("/Users/nickeishacuthbert/myfolder/Code")
...
CMS_ctrl <- Read10X('data_stress/M_CMS_ctrl')
baseline_ctrl <- Read10X('data_stress/M_baseline_ctrl')
CMS_ARS <- Read10X('data_stress/M_CMS_ARS')
baseline_ARS <- Read10X('data_stress/M_baseline_ARS')

# Create Seurat objects for each hypothalamus/stress conditions
# Exclude reads with <3 cells and cells with <200 genes
CMS_ctrl <- CreateSeuratObject(counts = CMS_ctrl, project = 'data_stress/M_CMS_ctrl', min.cells = 3, min.features = 200)
baseline_ctrl <- CreateSeuratObject(counts = baseline_ctrl, project = 'data_stress/M_baseline_ctrl', min.cells = 3, min.features = 200)
CMS_ARS <- CreateSeuratObject(counts = CMS_ARS, project = 'data_stress/M_CMS_ARS_ctrl', min.cells = 3, min.features = 200)
baseline_ARS <- CreateSeuratObject(counts = baseline_ARS, project = 'data_stress/M_baseline_ARS_ctrl', min.cells = 3, min.features = 200)
...
# First combine stress and young conditions for potential downstream analyses
...
# Then combine stress and old conditions for potential downstream analyses
